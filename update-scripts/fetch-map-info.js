...
const addBeatmapInfo = map => {
    ...
    : axios.get(urlBeatmapInfo(map.b)).then(({ data }) => {
        if (data.length > 0) {
          ...
          Object.keys(diff).forEach(key => {
            ...
            diff[key] =
              isNaN(parsed) || "last_update" === key
                ? diff[key]
                : truncateFloat(parsed);
          });
  ...
  return getPromise
    ...
    .catch(err => {
      ...
      return delay(1000).then(() => addBeatmapInfo(map));
    });
};
...
module.exports = () => {
  ...
  return mapsArray
    .reduce((promise, map, index) => {
      return promise.then(() => {
        index % 100 ||
          console.log(`Loading map #${index + 1}/${mapsArray.length}`);
        return addBeatmapInfo(map).then(() => {
          if (index % 500 === 0) {
            ...
            fs.writeFileSync(
              "result-array-with-info.json",
              JSON.stringify(arrayMaps)
            );
